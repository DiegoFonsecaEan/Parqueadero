...
public class Parqueadero {
    ...
    int mayor = 0;
    String placaCarroMayorTiempo = "";

    for (Tiempo t : puestos) {
        if (p.estaOcupado() && p.darMayorTiempo() > mayor ){
            mayor = p.darNumeroPlaca();
            placaCarroMayorTiempo = p.darCarro().darPlaca();
        }
    }
    ...
    public String metodo2() {
        ...
        for  (Puesto puesto : puestos){
            if(!puesto.estaOcupado()){
                puestosLibres = puestosLibres + 0;
                if (puestoslibres = 0) {
                    System.out.println("El parqueadero esta lleno");
            }
            }
                else if (puestosLibres > 0) {
                    System.out.println("Hay puestos disponibles");
        }
